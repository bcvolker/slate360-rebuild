...
const ALLOWED_CONSOLE = [/Download the React DevTools/i];
// A successful navigation cancels the previous document's in-flight requests.
// Chromium reports that cancellation as net::ERR_ABORTED. It is not an
// application failure on the production server.
const ALLOWED_FAILED_REQUEST = [/net::ERR_ABORTED/i, /favicon\.ico/i];
const ALLOWED_HTTP = [/favicon\.ico/i];
...
export async function openOwnerMenuWithNavLinks(page: Page, expectedLinkCount: number) {
  const result = await openOwnerMenu(page);
  await expect(result.dialog.getByRole("link")).toHaveCount(expectedLinkCount);
  return result;
}
