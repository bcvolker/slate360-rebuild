  ...
  return (
    <div className="rounded-xl bg-gray-50 border border-gray-100 p-3 flex-1 space-y-1.5">
      {display.slice(0, 5).map((ev, i) => {
        ...
        return (
          <div key={i} className={`flex items-center gap-2 rounded-lg px-2 py-1.5 ${isToday ? "bg-[color-mix(in_srgb,var(--primary)_10%,transparent)] border border-[color-mix(in_srgb,var(--primary)_20%,transparent)]" : isPast ? "opacity-50" : "bg-white border border-gray-100"}`}>
            ...
            <span className={`text-[9px] font-bold shrink-0 ${isToday ? "text-[var(--primary)]" : "text-gray-400"}`}>{isToday ? "Today" : d.toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
          </div>
        );
      })}
    </div>
  ...
  return (
    <div className="rounded-xl bg-gray-50 border border-gray-100 p-2 flex-1 space-y-1.5">
      {items.slice(0, 4).map((it, i) => (
        it.href ? (
          <a key={i} href={it.href} className="flex items-center gap-2 rounded-lg border border-gray-100 bg-white px-3 py-2 hover:border-[color-mix(in_srgb,var(--primary)_30%,transparent)] hover:bg-sky-50/30 transition group">
            ...
            <ChevronRight size={12} className="shrink-0 text-gray-300 group-hover:text-[var(--primary)] transition" />
          </a>
        ) : (
      ...
      {jobs.slice(0, 3).map((job) => (
        <div key={job.id} className="p-3 rounded-xl bg-gray-50 border border-gray-100">
          ...
          {job.status === "processing" && (
            <div className={bar}>
              <div className={fill} style={{ width: `${job.progress}%`, backgroundColor: "var(--primary)" }} />
            </div>
          )}
        </div>
      ))}
      ...
      {expanded && (
        <>
          <input
            ...
            className="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-xs focus:border-[var(--primary)] focus:ring-1 focus:ring-[var(--primary)] outline-none"
          />
          <textarea
            value={body}
            ...
            rows={3}
            className="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-xs focus:border-[var(--primary)] focus:ring-1 focus:ring-[var(--primary)] outline-none resize-none"
          />
        </>
      )}
      <button
    ...
    <div className="space-y-5 flex-1">
      <div>
        ...
        <div className={bar}>
          <div
            className={fill}
            style={{ width: `${creditPct}%`, backgroundColor: creditDanger ? "#EF4444" : "var(--primary)" }}
          />
        </div>
        {planLabel && (
      ...
      <div>
        ...
        <div className={bar}>
          <div className={fill} style={{ width: `${storagePct}%`, backgroundColor: "var(--primary)" }} />
        </div>
