    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div onClick={(e) => e.stopPropagation()} className="relative w-full max-w-3xl overflow-hidden rounded-2xl border border-zinc-700 bg-zinc-900 shadow-2xl text-white flex flex-col max-h-[90vh]">
        ...
        <div className="border-t border-zinc-800 bg-zinc-800/50 px-5 py-4 flex items-center justify-between">
          ...
          {step < TOTAL_STEPS ? (
            <button type="button" onClick={() => setStep((s) => s + 1)} disabled={!canAdvance} className="inline-flex items-center gap-1.5 rounded-xl bg-[#F59E0B] px-6 py-2.5 text-sm font-bold text-white hover:bg-[#E64500] disabled:opacity-50 transition-all">
              Next <ChevronRight size={14} />
            </button>
          ) : (
            <button type="submit" form="create-project-form" disabled={creating || !name.trim()} className="inline-flex items-center gap-2 rounded-xl bg-[#F59E0B] px-6 py-2.5 text-sm font-bold text-white hover:bg-[#E64500] disabled:opacity-50 transition-all">
