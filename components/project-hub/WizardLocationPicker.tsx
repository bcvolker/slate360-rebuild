...
function Controller({
      ...
      if (currentTool === "polygondraw") {
        ...
        if (previewPolylineRef.current) {
          ...
          previewPolylineRef.current = new google.maps.Polyline({
            path: newVerts, strokeColor: "#F59E0B", strokeWeight: 2, map,
          });
        }
  ...
  const finishPolygon = useCallback(() => {
    ...
    boundaryPolygonRef.current = new google.maps.Polygon({
      paths: verts, strokeColor: "#F59E0B", strokeWeight: 2,
      fillColor: "#F59E0B", fillOpacity: 0.15, editable: true, draggable: true, map,
    });
  ...
  const btn = (active: boolean) =>
    `px-2 py-1.5 rounded-lg text-xs font-bold transition-colors flex items-center gap-1 ${
      active ? "bg-[#F59E0B] text-white" : "bg-white text-gray-600 hover:bg-gray-50 border border-gray-200"
    }`;
    ...
    <div className="absolute inset-0 pointer-events-none">
      ...
      <div className="absolute top-2 left-2 right-2 z-10 pointer-events-auto">
        <div className="flex gap-1.5 bg-white rounded-xl shadow-md border border-gray-200 p-1.5">
          ...
          <button type="button" onClick={() => void searchAddress()} className="px-2.5 py-1.5 bg-[#F59E0B] hover:bg-[#E64500] rounded-lg text-white transition-colors"><Search size={14} /></button>
        </div>
      </div>
      ...
      {!isDrawingPolygon && value.boundary.length > 0 && (
        <div className="absolute top-14 right-2 z-10 pointer-events-none">
          <div className="bg-[#F59E0B]/10 border border-[#F59E0B]/30 rounded-lg px-2 py-1 text-[10px] text-[#F59E0B] font-semibold">Boundary: {value.boundary.length} pts</div>
        </div>
      )}
    </div>
  );
