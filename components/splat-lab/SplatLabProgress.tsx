    ...
    <div className="grid gap-4 lg:grid-cols-[1fr_1fr]">
      <div className="rounded-xl border border-white/10 bg-white/[0.04] p-4 backdrop-blur">
        <div className="flex items-center justify-between">
          <p className="font-mono text-[10px] uppercase tracking-wide text-[var(--graphite-muted)]">Pipeline</p>
        ...
        {job?.manifest ? (
          <details className="mt-3">
            ...
            <pre className="mt-2 overflow-auto rounded-md border border-white/10 bg-[var(--graphite-canvas)] p-2 text-[10px] text-zinc-400">{JSON.stringify(job.manifest, null, 2)}</pre>
          </details>
        ) : null}
      </div>

      <div className="relative min-h-[320px] rounded-xl border border-white/10 bg-white/[0.04] backdrop-blur">
        <p className="absolute left-3 top-3 z-10 font-mono text-[10px] uppercase tracking-wide text-[var(--graphite-muted)]">Preview</p>
        {modelUrl ? (
          ...
          <div className="flex h-full min-h-[320px] flex-col items-center justify-center gap-2 p-6 text-center">
            {status === "running" || status === "queued" ? (
              <>
                ...
                <p className="text-[10px] text-zinc-600">Slice 1 extracts frames only; SfM + training arrive in Slice 2.</p>
              </>
            ) : (
