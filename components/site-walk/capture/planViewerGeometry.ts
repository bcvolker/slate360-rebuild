...
export function calculateCenteredPlanTransform({ maxScale, minScale, padding, surface, viewport }: FitPlanOptions) {
  ...
  return { scale: nextScale, x: (viewportWidth - surfaceWidth * nextScale) / 2, y: (viewportHeight - surfaceHeight * nextScale) / 2 };
}