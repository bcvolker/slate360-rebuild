...
export function PhotoAttachmentFilePreviewModal({ file, onClose }: Props) {
  ...
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/76 p-4" role="dialog" aria-label={`Preview ${file.name}`} onClick={onClose}>
      <div className="relative flex max-h-[78dvh] w-full max-w-lg flex-col overflow-hidden rounded-3xl border border-white/10 bg-slate-950 shadow-2xl" onClick={(event) => event.stopPropagation()}>
        <button type="button" onClick={onClose} className="absolute right-2 top-2 z-10 rounded-full border border-white/25 bg-black/80 p-2 text-white shadow-xl" aria-label="Close preview"><X className="h-5 w-5" /></button>
        <div className="flex items-center justify-between gap-3 border-b border-white/10 px-3 py-2">
          <p className="min-w-0 pr-12 truncate text-sm font-black text-white">{file.name}</p>
        </div>
        <div className="h-[54dvh] min-h-64 flex-1 overflow-hidden bg-black">
          {preview.loading ? <div className="flex h-full items-center justify-center text-white"><Loader2 className="h-6 w-6 animate-spin" /></div> : preview.error ? <div className="flex h-full items-center justify-center p-6 text-center text-sm font-bold text-rose-100">{preview.error}</div> : isImage ? <div className="flex h-full touch-none items-center justify-center overflow-hidden" onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={handlePointerUp}><img src={preview.url ?? ""} alt={file.name} className="max-h-full max-w-full object-contain" style={{ transform: `translate(${zoom.x}px, ${zoom.y}px) scale(${zoom.scale})`, transformOrigin: "center" }} draggable={false} /></div> : <iframe src={preview.url ?? ""} title={file.name} className="h-full w-full bg-white" />}
        </div>
