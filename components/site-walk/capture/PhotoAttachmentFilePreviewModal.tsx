...
export function PhotoAttachmentFilePreviewModal({ file, onClose }: Props) {
  ...
  function handlePointerMove(event: ReactPointerEvent<HTMLDivElement>) {
    ...
    if (pointersRef.current.size === 2 && pinchRef.current) {
      ...
      setZoom((current) => ({ ...current, scale: clamp((distance(a, b) / pinch.distance) * pinch.scale, 1, 4) }));
      return;
    }
  ...
  return (
    <div className="absolute inset-0 z-50 flex touch-none items-center justify-center bg-black/76 p-3" style={{ touchAction: "none" }} role="dialog" aria-label={`Preview ${file.name}`} onClick={onClose} onPointerDown={(event) => event.stopPropagation()} onPointerMove={(event) => event.stopPropagation()} onPointerUp={(event) => event.stopPropagation()} onWheel={(event) => event.stopPropagation()}>
      <div className="relative flex flex-col overflow-hidden rounded-3xl border border-white/10 bg-slate-950 shadow-2xl" style={{ width: "min(17rem, calc(100% - 1rem))", height: "min(20rem, calc(100% - 1rem))" }} onClick={(event) => event.stopPropagation()}>
        ...
        <div className="min-h-0 flex-1 overflow-hidden bg-black">
          {preview.loading ? <div className="flex h-full items-center justify-center text-white"><Loader2 className="h-6 w-6 animate-spin" /></div> : preview.error ? <div className="flex h-full items-center justify-center p-6 text-center text-sm font-bold text-rose-100">{preview.error}</div> : isImage ? <div className="flex h-full touch-none items-center justify-center overflow-hidden" style={{ touchAction: "none" }} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={handlePointerUp}><img src={preview.url ?? ""} alt={file.name} className="max-h-[60vh] max-w-full object-contain" style={{ transform: `translate(${zoom.x}px, ${zoom.y}px) scale(${zoom.scale})`, transformOrigin: "center" }} draggable={false} /></div> : <iframe src={preview.url ?? ""} title={file.name} className="h-full w-full bg-white" />}
        </div>
        <div className="flex shrink-0 items-center justify-between gap-2 border-t border-white/10 px-3 py-2"><p className="text-[10px] font-bold text-white/50">Pinch image to zoom.</p>{preview.url && <a href={preview.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-xs font-black text-cyan-100"><ExternalLink className="h-4 w-4" /> Open</a>}</div>
      </div>
    </div>
  );
}
