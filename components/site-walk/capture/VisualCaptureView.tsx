...
export function VisualCaptureView({ sessionId, autoOpenCamera, launchId, items, activeItemId, ghostImageUrl, onMarkupChange, onAttachmentPinsChange, onSelectItem, onNext }: Props) {
  ...
  return (
    <div className="flex h-[100dvh] w-full flex-col overflow-hidden bg-zinc-950 text-white">
      ...
      {/* Image canvas — pinch/zoom/pan + long-press pin handled inside PhotoMarkupCanvas */}
      <main className="relative min-h-0 flex-1 overflow-hidden">
        ...
        {activeItem && (
          <button type="button" onClick={() => setAttachmentsOpen(true)} className="absolute bottom-3 left-3 z-30 inline-flex items-center gap-2 rounded-full border border-white/20 bg-black/60 px-3 py-2 text-xs font-black text-white shadow-xl backdrop-blur-xl" aria-label="Open pinned attachments">
            <Paperclip className="h-4 w-4 text-blue-300" /> Attachments ({pinCount})
          </button>
        )}
        {activeItem && (
          <button type="button" onClick={onNext} className="absolute right-2 top-1/2 z-20 flex h-28 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-white/15 bg-black/35 text-white/80 shadow-xl backdrop-blur-xl" aria-label="Swipe right to notes">
      ...
      <div className="shrink-0 border-t border-white/10 bg-black/65 px-3 py-2 backdrop-blur-xl">
        ...
        {/* Angles carousel (bottom 1) */}
        <div className="mt-2 flex h-20 gap-2 overflow-x-auto whitespace-nowrap pb-1 no-scrollbar" aria-label="Current stop angles">
        ...
        {/* Progress carousel (bottom 2) */}
        {activeItem && (
          <ProgressTimeline
            items={progressItems}
            ghostOn={ghostOn}
            onToggleGhost={() => setGhostOn((current) => !current)}
            onAdd={() => { setGhostOn(true); requestCameraCapture("camera", "next_item"); }}
            onSelectItem={onSelectItem}
            ghostAvailable={!!ghostImageUrl}
          />
        )}
      </div>
    </div>
...
function ProgressTimeline({ items, ghostOn, ghostAvailable, onToggleGhost, onAdd, onSelectItem }: { items: CaptureItemRecord[]; ghostOn: boolean; ghostAvailable: boolean; onToggleGhost: () => void; onAdd: () => void; onSelectItem: (item: CaptureItemRecord) => void }) {
  return (
    <div className="relative mt-2 overflow-hidden rounded-2xl border border-white/10 bg-black/80 px-2 py-2">
      ...
      <div className="flex h-16 gap-2 overflow-x-auto whitespace-nowrap px-5 pb-1 no-scrollbar" aria-label="Progress timeline">
    ...
    <div className="absolute inset-0 z-40 flex flex-col justify-end bg-black/55" role="dialog" aria-label="Pinned attachments" onClick={onClose}>
      <div className="rounded-t-3xl border-t border-white/10 bg-zinc-950/95 p-3 shadow-2xl backdrop-blur-xl" onClick={(event) => event.stopPropagation()}>
        <div className="mb-2 flex items-center justify-between">
          <p className="text-[11px] font-black uppercase tracking-[0.16em] text-blue-100">Pinned attachments ({pins.length})</p>
          <button type="button" onClick={onClose} className="rounded-full border border-white/15 p-2 text-white/80" aria-label="Close attachments"><X className="h-4 w-4" /></button>
        </div>
        {pins.length === 0 ? (
          <p className="rounded-2xl border border-dashed border-white/15 bg-white/5 px-4 py-6 text-center text-xs font-bold text-white/60">Long-press the photo to drop a pin and attach files.</p>
        ) : (
          <ul className="max-h-64 space-y-2 overflow-y-auto pr-1 no-scrollbar">
            {pins.map((pin) => (
              <li key={pin.id} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-3 py-2">
                <Paperclip className="h-4 w-4 shrink-0 text-blue-300" />
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-black text-white">{pin.label}</p>
                  <p className="truncate text-[11px] font-bold text-white/60">{pin.files.length} file{pin.files.length === 1 ? "" : "s"}{pin.note ? ` · ${pin.note}` : ""}</p>
                </div>
                <button type="button" onClick={() => onRemove(pin.id)} className="rounded-full border border-white/15 p-2 text-white/75 hover:border-rose-400 hover:text-rose-200" aria-label={`Remove ${pin.label}`}><Trash2 className="h-4 w-4" /></button>
              </li>
            ))}
          </ul>
        )}
...
function StopStrip({ items, activeItemId, onSelectItem, onOpenEdit, onUndo, onRedo }: { items: CaptureItemRecord[]; activeItemId: string | null; onSelectItem: (item: CaptureItemRecord) => void; onOpenEdit: () => void; onUndo: () => void; onRedo: () => void }) {
  return (
    <div className="relative shrink-0 overflow-hidden border-b border-white/10 bg-black px-2 py-2">
      <div className="pointer-events-none absolute inset-y-0 left-0 z-10 flex w-8 items-center justify-start bg-gradient-to-r from-black to-transparent pl-1 text-white/30"><ChevronLeft className="h-4 w-4" /></div>
      <div className="pointer-events-none absolute inset-y-0 right-0 z-10 flex w-8 items-center justify-end bg-gradient-to-l from-black to-transparent pr-1 text-white/30"><ChevronRight className="h-4 w-4" /></div>
      <div className="flex h-20 gap-2 overflow-x-auto whitespace-nowrap px-5 pb-1 no-scrollbar" aria-label="Stops in this walk">
