...
export function PendingUploadPreviewModal({ fileName, imageUrl, busy, errorMessage, onCancel, onConfirmAttach }: Props) {
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/80 p-4" role="dialog" aria-label="Confirm uploaded image" onClick={() => { if (!busy) onCancel(); }}>
      <div className="w-full max-w-3xl rounded-[2rem] border border-white/10 bg-slate-950 p-4 shadow-2xl" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <button type="button" onClick={(event) => { event.stopPropagation(); void onConfirmAttach(); }} disabled={busy} className="min-h-12 rounded-2xl bg-amber-500 px-4 py-3 text-sm font-black text-slate-950 hover:bg-amber-400 disabled:opacity-60">{busy ? "Attaching…" : "Confirm & Attach"}</button>
          <button type="button" onClick={(event) => { event.stopPropagation(); onCancel(); }} disabled={busy} className="min-h-12 rounded-2xl border border-white/15 bg-white/[0.04] px-4 py-3 text-sm font-black text-white hover:border-amber-300/50 disabled:opacity-50">Cancel</button>
        </div>
      </div>
    </div>
  );
