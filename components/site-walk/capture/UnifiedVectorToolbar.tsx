...
const COLORS = ["#D4AF37", "#3B82F6", "#ef4444", "#a855f7", "#f8fafc"];
    ...
    <section className="rounded-3xl border border-white/15 bg-white/10 p-2 shadow-sm backdrop-blur-xl" aria-label="Vector markup toolbar">
      ...
      <div className="grid grid-cols-6 gap-1">
        {TOOLS.map((tool) => {
          ...
          return (
            <button key={tool.label} type="button" onClick={() => selectTool(tool.value)} className={`flex min-h-10 items-center justify-center gap-2 rounded-xl border px-2 py-2 text-xs font-black ${isActive ? "border-[#D4AF37] bg-[#D4AF37]/20 text-[#F8E7A1]" : "border-white/10 bg-black/20 text-white/70 hover:border-[#D4AF37]/60"}`} aria-label={tool.label}>
      ...
      <div className="mt-2 flex flex-wrap gap-2" aria-label="Markup colors">
        {COLORS.map((color) => <button key={color} type="button" onClick={() => selectColor(color)} className={`h-7 w-7 rounded-full border-2 ${activeColor === color ? "border-[#D4AF37]" : "border-white/40 shadow"}`} style={{ backgroundColor: color }} aria-label={`Use markup color ${color}`} />)}
      </div>
