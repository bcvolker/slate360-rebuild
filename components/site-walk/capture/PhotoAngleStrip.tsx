  ...
  return (
    <GlassCard className={`${className} bg-slate-950/65 p-2 backdrop-blur-xl`}>
      <div className="flex items-center gap-2 overflow-x-auto no-scrollbar" aria-label="Current item photo angles">
        ...
        <button type="button" onClick={onAddAngle} className="flex h-16 min-w-28 shrink-0 flex-col items-center justify-center gap-1 rounded-2xl border border-amber-300/50 bg-amber-500/15 px-3 text-[11px] font-black uppercase tracking-[0.08em] text-amber-100 hover:bg-amber-500/25" aria-label="Add another angle to this item">
...
function AngleButton({ label, imageUrl, active, failed = false, onClick }: { label: string; imageUrl: string | null; active: boolean; failed?: boolean; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className={`relative h-16 w-16 shrink-0 overflow-hidden rounded-2xl border bg-black/60 ${active ? "border-amber-400 ring-2 ring-amber-500/30" : "border-white/15"}`} aria-label={`Open ${label}`}>
