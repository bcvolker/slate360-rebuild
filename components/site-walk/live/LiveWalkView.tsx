    ...
    <section className="grid min-h-[calc(100vh-12rem)] gap-4 xl:grid-cols-[420px_minmax(0,1fr)]">
      <aside className="overflow-hidden rounded-3xl border border-slate-300 bg-white shadow-sm">
        <div className="border-b border-slate-200 p-4">
          <p className="text-xs font-black uppercase tracking-[0.18em] text-[var(--graphite-primary)]">Live Feed</p>
        ...
        <div className="max-h-[calc(100vh-18rem)] overflow-y-auto p-3">
          {groups.map((group) => (
            <div key={group.location} className="mb-4">
              <div className="mb-2 flex items-center gap-2 rounded-2xl bg-slate-100 px-3 py-2 text-sm font-black text-slate-800"><MapPin className="h-4 w-4 text-[var(--graphite-primary)]" /> {group.location}</div>
              <div className="space-y-2">
                {group.items.map((item) => {
                  const active = selected?.id === item.id;
                  const recent = recentIds.has(item.id);
                  return (
                    <button key={item.id} type="button" onClick={() => setSelectedId(item.id)} className={`w-full rounded-2xl border p-3 text-left transition ${active ? "border-[var(--graphite-primary)] bg-[color-mix(in_srgb,var(--graphite-primary)_10%,transparent)]" : "border-slate-200 bg-white hover:border-[color-mix(in_srgb,var(--graphite-primary)_40%,transparent)]"} ${recent ? "animate-pulse ring-2 ring-emerald-300" : ""}`}>
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 border-b border-slate-200 pb-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.18em] text-[var(--graphite-primary)]">Live item</p>
...
function EmptyDetail() {
  return <div className="flex min-h-96 flex-col items-center justify-center text-center text-slate-600"><Activity className="h-8 w-8 text-[var(--graphite-primary)]" /><p className="mt-3 text-sm font-black">Waiting for live field data.</p></div>;
}
