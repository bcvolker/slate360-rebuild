...
export default function TileSection({ id, title, subtitle, description, features, learnHref, viewerOn = "right", alt = false, hero = false, viewerStyle, viewerLeft = false }: Props) {
  ...
  return (
    <section
      ...
      data-bg={bg}
    >
      <div className={`mx-auto max-w-7xl w-full px-6 md:px-10 py-24 flex ${viewerOn === 'left' ? 'flex-row-reverse' : 'flex-row'} gap-6 items-center min-w-0`}>
        <div className="flex-shrink-0 w-[40%] h-[60vh] flex items-center justify-center">
          <MediaViewer hero={hero} style={viewerStyle} alt={alt} />
        </div>
        <div className={`flex-1 min-w-0 flex flex-col space-y-4 p-8 ${alt ? '' : 'text-white'}`}> 
