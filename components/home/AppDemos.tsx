...
import { Play, Camera, Building2, Globe, Layers, Eye } from "lucide-react";
import { Card } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import type { AppItem } from "./landing-data";
  ...
  return (
    <Card className="bg-glass border-glass shadow-glass overflow-hidden">
      ...
      >
        ...
        >
          <div className="h-8 w-8 rounded-full bg-primary/80 border-2 border-white flex items-center justify-center animate-pulse cursor-pointer hover:scale-110 transition-transform">
...
function ThreeDDemo() {
  ...
  return (
    <Card className="bg-glass border-glass shadow-glass overflow-hidden">
      <div
        className="relative aspect-video cursor-grab active:cursor-grabbing select-none flex items-center justify-center"
      ...
      >
        ...
        >
          <div
            className="absolute inset-0 bg-gradient-to-b from-zinc-600 to-zinc-800 border border-primary/30"
            style={{ transform: "translateZ(40px)" }}
          >
            <div className="grid grid-cols-3 gap-2 p-4">
              {[...Array(9)].map((_, i) => (
                <div key={i} className="aspect-square bg-primary/20 border border-primary/30" />
              ))}
            </div>
          </div>
          <div
          ...
          />
          <div
            className="absolute top-0 left-0 w-20 h-full bg-gradient-to-b from-zinc-500 to-zinc-700 border-r border-primary/20"
            style={{ transform: "rotateY(-90deg) translateZ(0px)" }}
          />
          <div
            className="absolute top-0 right-0 w-20 h-full bg-gradient-to-b from-zinc-500 to-zinc-700 border-l border-primary/20"
            style={{ transform: "rotateY(90deg) translateZ(152px)" }}
          />
          <div
            className="absolute top-0 left-0 w-full h-20 bg-primary/30"
