...
const SORT_OPTIONS: Array<{ key: MarketSortKey; label: string }> = [
  { key: "edge", label: "Best opportunity" },
...
export default function MarketDirectBuyResults({ markets, sortBy, sortDirection, tableInsights, onToggleSort, onBuy, onOpenDetails, savedMarketIds = [], onToggleSave }: Props) {
  ...
  return (
    <>
      <div className="flex flex-col gap-3 rounded-xl border border-zinc-800 bg-zinc-950/80 p-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-wrap items-center gap-3 text-sm text-slate-300">
          <span className="rounded-lg border border-zinc-800 bg-zinc-900 px-3 py-1 font-semibold text-slate-100">{markets.length} markets</span>
          <span>{highQualityCount} strong opportunities</span>
          <span>Best value {tableInsights.topEdge.toFixed(1)}%</span>
          <span>Avg activity ${Math.round(tableInsights.averageVolume).toLocaleString()}</span>
        </div>
        <div className="flex flex-wrap gap-2">
          {SORT_OPTIONS.map((option) => (
            <button
              ...
              className={`rounded-full border px-3 py-1.5 text-xs font-semibold transition ${sortBy === option.key ? "border-cyan-400/40 bg-cyan-500/20 text-cyan-100" : "border-zinc-800 bg-zinc-900 text-slate-300 hover:border-zinc-700 hover:text-slate-100"}`}
            >
      ...
      <div className="mt-2 overflow-hidden rounded-xl border border-zinc-800 bg-zinc-950/85">
        {markets.length === 0 ? (
          <p className="py-12 text-center text-sm text-slate-400">No markets match your filters.</p>
        ) : (
          <div className="max-h-[560px] overflow-auto">
            <table className="min-w-full text-left text-sm text-slate-200">
              <thead className="sticky top-0 z-10 bg-zinc-900/95 text-xs uppercase tracking-[0.16em] text-slate-400">
                <tr>
...
function MarketRow({ market, onBuy, onOpenDetails, isSaved, onToggleSave }: { market: MarketListing; onBuy: (market: MarketListing, outcome: "YES" | "NO") => void; onOpenDetails: (market: MarketListing) => void; isSaved: boolean; onToggleSave?: (market: MarketListing) => void }) {
  return (
    <tr className="border-t border-zinc-800/80 bg-zinc-950/20 transition hover:bg-zinc-900/40" onClick={() => onOpenDetails(market)}>
      <td className="px-4 py-3 align-top">
        <div className="space-y-1">
          <p className="line-clamp-2 max-w-[460px] font-semibold text-slate-100">{market.title}</p>
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-400">
            <span className="rounded-lg border border-zinc-800 bg-zinc-900 px-2 py-0.5">{market.category}</span>
      ...
      <td className="px-4 py-3" onClick={(event) => event.stopPropagation()}>
        <div className="flex items-center justify-end gap-1.5">
          <button onClick={() => onToggleSave?.(market)} className={`rounded-full border px-2.5 py-1 text-[11px] font-semibold transition ${isSaved ? "border-amber-400/35 bg-amber-500/20 text-amber-100" : "border-zinc-800 bg-zinc-900 text-slate-300 hover:border-zinc-700 hover:text-slate-100"}`}>
            {isSaved ? "Saved" : "Save"}
          </button>
          <button onClick={() => onBuy(market, "YES")} className="rounded-lg border border-emerald-400/35 bg-emerald-500/15 px-2.5 py-1 text-[11px] font-semibold text-emerald-100 transition hover:bg-emerald-500/25">Buy YES</button>
          <button onClick={() => onBuy(market, "NO")} className="rounded-lg border border-rose-400/35 bg-rose-500/15 px-2.5 py-1 text-[11px] font-semibold text-rose-100 transition hover:bg-rose-500/25">Buy NO</button>
        </div>
      </td>
    </tr>
  );
