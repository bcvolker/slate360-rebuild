...
function planHealthTone(warnings: number) {
  if (warnings >= 3) return "text-red-700 bg-red-50 border-red-200";
  if (warnings >= 1) return "text-amber-700 bg-amber-50 border-amber-200";
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">Plan sanity check</p>
          <p className="mt-1 text-sm text-slate-600">A quick read on whether this plan is likely to feel stable and understandable.</p>
        </div>
      ...
      <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-3">
        <div className="rounded-xl border border-white bg-white px-3 py-3">
          ...
          <p className="mt-1 text-lg font-bold text-slate-900">${budgetPerPosition.toFixed(2)}</p>
        </div>
        <div className="rounded-xl border border-white bg-white px-3 py-3">
          <p className="text-xs text-slate-400">Average trade cadence</p>
          <p className="mt-1 text-lg font-bold text-slate-900">{hoursBetweenTrades >= 1 ? `${hoursBetweenTrades.toFixed(1)}h` : `${Math.round(hoursBetweenTrades * 60)}m`}</p>
        </div>
        <div className="rounded-xl border border-white bg-white px-3 py-3">
          <p className="text-xs text-slate-400">Capital protection</p>
          <p className="mt-1 text-lg font-bold text-slate-900">${draft.maxDailyLoss.toFixed(0)} / day</p>
        </div>
      </div>
      ...
      {warnings.length > 0 ? (
        <div className="mt-3 space-y-2">
          {warnings.map((warning) => (
            <p key={warning} className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700">{warning}</p>
          ))}
        </div>
      ) : (
