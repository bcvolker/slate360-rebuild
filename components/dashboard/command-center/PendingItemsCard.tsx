          ...
          <>
            {activeProjects.slice(0, 5).map((p) => (
              <a
                ...
                className="flex items-center gap-3 rounded-lg bg-white/[0.04]/50 px-4 py-3 hover:bg-amber-500/10 transition-colors group"
              >
                <div className="h-8 w-8 rounded-lg bg-primary/20 flex items-center justify-center flex-shrink-0">
                  <FolderOpen className="h-4 w-4 text-primary" />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium text-foreground group-hover:text-amber-300 truncate">{p.name}</p>
