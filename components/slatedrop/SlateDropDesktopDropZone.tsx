  ...
  return (
    <section
      ...
      className={`hidden rounded-3xl border-2 border-dashed p-6 text-center shadow-lg backdrop-blur-md transition-colors lg:block ${
        state === "dragging" ? "border-[color-mix(in_srgb,var(--graphite-primary)_40%,transparent)] bg-[color-mix(in_srgb,var(--graphite-primary)_10%,transparent)]" : "border-white/15 bg-white/[0.03]"
      }`}
    >
      <input
        ref={inputRef}
      ...
      />
      <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-[color-mix(in_srgb,var(--graphite-primary)_10%,transparent)] text-[var(--graphite-primary)] ring-1 ring-[color-mix(in_srgb,var(--graphite-primary)_20%,transparent)]">
      ...
      <h2 className="mt-3 text-base font-black text-[var(--graphite-text-header)]">{label}</h2>
      <p className="mx-auto mt-1 max-w-xl text-sm leading-6 text-[var(--graphite-muted)]">{message}</p>
      {state === "uploading" && <p className="mt-2 text-xs font-bold text-[var(--graphite-primary)]">{uploaded} uploaded so far</p>}
      <button
        type="button"
        onClick={() => inputRef.current?.click()}
        disabled={state === "uploading"}
        className="mt-4 inline-flex min-h-10 items-center justify-center rounded-2xl bg-[var(--graphite-primary)] px-4 text-xs font-black text-[var(--graphite-canvas)] hover:opacity-90 disabled:opacity-60"
      >
        Choose files
      </button>
    </section>
