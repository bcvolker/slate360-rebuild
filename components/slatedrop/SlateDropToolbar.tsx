...
export default function SlateDropToolbar({
  ...
  return (
    <div className="shrink-0 bg-zinc-950 border-b border-zinc-800 px-3 sm:px-4 py-3">
      {/* Row 1: Breadcrumb + upload button */}
      <div className="flex items-center gap-2 min-w-0">
        <button
          onClick={onToggleSidebar}
          className="hidden md:flex w-7 h-7 rounded-lg items-center justify-center text-zinc-500 hover:bg-zinc-800 shrink-0"
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2">
        <div className="relative w-full sm:min-w-0 sm:flex-1">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-2 rounded-lg border border-zinc-700 bg-zinc-900 text-xs text-zinc-100 placeholder:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-[#F59E0B]/20 focus:border-[#F59E0B] transition-all"
          />
        </div>

        <button
          onClick={onCycleSort}
          className="w-9 h-9 rounded-lg border border-zinc-700 flex items-center justify-center text-zinc-400 hover:bg-zinc-800 transition-colors shrink-0"
        ...
        <div className="flex rounded-lg border border-zinc-700 overflow-hidden shrink-0">
          <button
            ...
            className={`w-9 h-9 flex items-center justify-center transition-colors ${viewMode === "grid" ? "bg-[#F59E0B] text-zinc-950" : "text-zinc-400 hover:bg-zinc-800"}`}
          >
            <Grid3X3 size={14} />
          </button>
          <button
            onClick={() => onSetViewMode("list")}
            className={`w-9 h-9 flex items-center justify-center transition-colors ${viewMode === "list" ? "bg-[#F59E0B] text-zinc-950" : "text-zinc-400 hover:bg-zinc-800"}`}
          >
            <List size={14} />
          </button>
        </div>
        ...
        {showZipButton && (
          <button
            ...
            className="hidden sm:flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold border border-zinc-700 text-zinc-400 hover:bg-zinc-800 transition-colors shrink-0"
