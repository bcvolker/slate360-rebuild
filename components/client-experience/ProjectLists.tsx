...
import { formatDate, formatTime, ITEM_STATUS_LABEL, ITEM_TYPE_LABEL, stationsForVisit } from "@/lib/client-experience/utils";
  ...
  return (
    <main className="ce-page" data-testid="ce-reality">
      <section className="ce-section">
        ...
        <p className="ce-body" style={{ maxWidth: "58ch", marginTop: 12 }}>Move between the continuous walkthrough, the 3D twin and the dated 360 stations. Items and documents follow you into each one.</p>
          ...
          {data.visits.map((v) => {
            ...
            return (
              <div key={v.id} id={v.id} className="ce-row" style={{ gridTemplateColumns: "140px 1fr auto", padding: "18px 4px" }}>
                <img src={v.thumbUrl} alt="" width={140} height={70} style={{ borderRadius: 4, objectFit: "cover" }} />
                <div>
                  <div className="ce-row__title">{formatDate(v.capturedAt)} · {formatTime(v.capturedAt)}</div>
                  <div className="ce-row__sub">{v.label}</div>
                  <div className="ce-row__sub" style={{ marginTop: 6 }}>{v.modalities.map((m) => MODALITY_LABEL[m]).join(" · ")}{stations.length ? ` · ${stations.length} stations` : ""}</div>
                </div>
                <div className="ce-row__side">
                  {v.modalities.includes("walkthrough") ? <Link href={`${base}/walk`} className="ce-btn ce-btn--sm">Walkthrough</Link> : null}
                  {stations.length ? <Link href={`${base}/stations?s=${stations[0].id}`} className="ce-btn ce-btn--sm">360</Link> : null}
                  {v.modalities.includes("twin") ? <Link href={`${base}/twin`} className="ce-btn ce-btn--sm">Twin</Link> : null}
                </div>
              </div>
            );
          })}
          ...
          {data.documents.map((d) => {
            ...
            return (
              <div key={d.id} id={d.id} className="ce-tile">
                <a href={d.url} target="_blank" rel="noreferrer"><img src={d.thumbUrl} alt="" className="ce-tile__img" /></a>
                <div className="ce-tile__body">
                  ...
                  {refs.length ? (
                    <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 4 }}>
                      {refs.map((i) => (
                        <Link key={i.id} href={`${base}/items/${i.id}`} className="ce-body" style={{ fontSize: 13 }}>Referenced by “{i.title}”</Link>
                      ))}
                    </div>
                  ) : null}
                  {d.sheetId ? <Link href={`${base}/plan`} className="ce-btn ce-btn--sm" style={{ marginTop: 12 }}>Open on plan</Link> : null}
                </div>
              </div>
            );
          })}
...
export function ItemsList({ data }: { data: ProjectExperience }) {
  ...
  return (
    <main className="ce-page" data-testid="ce-items">
      <section className="ce-section">
        <div className="ce-section__head"><h2 className="ce-h2">Project items</h2><span className="ce-section__link">{data.items.filter((i) => i.status !== "resolved").length} open</span></div>
        <div className="ce-list">
          {data.items.map((i) => (
            <Link key={i.id} href={`${base}/items/${i.id}`} className="ce-row" style={{ gridTemplateColumns: "1fr auto auto" }}>
              <div>
                <div className="ce-row__title">{i.title}</div>
                <div className="ce-row__sub">{ITEM_TYPE_LABEL[i.type]} · opened {formatDate(i.createdAt)} · {i.refs.length} locations{i.comments.length ? ` · ${i.comments.length} comments` : ""}</div>
              </div>
