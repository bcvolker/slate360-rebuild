...
export default function ContactPicker({
  ...
  const inputClasses =
    "w-full px-3.5 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#D4AF37]/20 focus:border-[#D4AF37] transition-all disabled:opacity-50 disabled:cursor-not-allowed";
  ...
  return (
    <div ref={wrapperRef} className={`relative ${className}`}>
      <button
        onClick={handleOpen}
        disabled={disabled}
        className="flex items-center gap-2 px-3.5 py-2 rounded-xl border border-gray-200 text-sm text-gray-600 hover:border-[#D4AF37]/50 hover:text-[#D4AF37] transition-all disabled:opacity-50"
      >
        <UserPlus size={14} />
        Pick contact
            ...
            <div className="p-3 border-b border-gray-100">
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) => handleQueryChange(e.target.value)}
                  placeholder="Search…"
                  className="w-full pl-8 pr-3 py-2 rounded-lg border border-gray-200 text-xs focus:outline-none focus:ring-2 focus:ring-[#D4AF37]/20 focus:border-[#D4AF37]"
                />
              </div>
            </div>
            <ContactDropdown
    ...
    return (
      <div className={base}>
        <div className="p-4 text-center">
          <div className="w-4 h-4 border-2 border-[#D4AF37]/30 border-t-[#D4AF37] rounded-full animate-spin mx-auto" />
        </div>
      </div>
    );
  }
