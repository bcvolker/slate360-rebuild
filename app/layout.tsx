...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  <body className={`bg-white text-gray-900 overflow-x-hidden md:overflow-hidden ${inter.className}`}>
    <Navbar />
    <DebugScrollOverlay />
  <main id="scroll-container" className="h-auto md:h-[calc(100vh-5rem)] overflow-visible md:overflow-y-scroll bg-white overscroll-contain touch-pan-y">{children}</main>
      </body>
    </html>
  );
}
