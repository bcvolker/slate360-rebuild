...
export default async function DigitalTwinUploadPage({ searchParams }: PageProps) {
  ...
  // Terrestrial LiDAR scans keep their dedicated desktop panel (Track L) —
  // the unified Review & Sources screen is the photo/video/phone flow.
  if (params.mode === "lidar") {
    const lockProject = Boolean(initialProjectId);
    return (
