  ...
  try {
    ...
    if (lowerName.endsWith(".insv") || lowerName.endsWith(".360") || lowerName.endsWith(".dng")) {
      return NextResponse.json(
        { 
          error: "Raw camera files are not supported. Please export your panorama to a finished JPEG (equirectangular format) using your camera's desktop or mobile app first." 
        }, 
        { status: 400 }
      );
    }
    ...
    const s3Key = `tours/${ctx.org.id}/${params.tourId}/scenes/${timestamp}_${cleanFilename}`;
    ...
    return NextResponse.json({ uploadUrl, s3Key, size });
  } catch (err: any) {
