    ...
    <>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="relative">
            ...
            <input
              ...
              className="rounded-lg border border-zinc-700 bg-card py-1.5 pl-8 pr-3 text-xs text-zinc-200 placeholder:text-zinc-500 focus:border-[var(--primary)] focus:outline-none focus:ring-1 focus:ring-[color-mix(in_srgb,var(--primary)_20%,transparent)] w-52"
            />
          </div>
          <select
            value={categoryFilter}
            onChange={(e) => setCategoryFilter(e.target.value)}
            className="rounded-lg border border-zinc-700 bg-card px-2.5 py-1.5 text-xs text-zinc-300 focus:border-[var(--primary)] focus:outline-none"
          >
            {categories.map((c) => (
        ...
        <div className="flex items-center gap-2">
          {selectedIds.size > 0 && (
            <div className="flex items-center gap-1.5 mr-2">
              <span className="text-xs font-semibold text-[var(--primary)]">{selectedIds.size} selected</span>
              <button onClick={handleDownloadSelected} className="rounded-md border border-[color-mix(in_srgb,var(--primary)_30%,transparent)] bg-[color-mix(in_srgb,var(--primary)_10%,transparent)] px-2 py-1 text-xs font-semibold text-[var(--primary)] hover:bg-[color-mix(in_srgb,var(--primary)_20%,transparent)]">
          ...
          {([ ["grid", Grid3X3], ["masonry", Image], ["list", List] ] as const).map(([mode, Icon]) => (
            <button
              ...
              className={`rounded-md p-1.5 transition ${viewMode === mode ? "bg-[var(--primary)] text-foreground" : "border border-zinc-700 bg-card text-zinc-400 hover:bg-zinc-700"}`}
      ...
      {selectedIds.size === 0 && (
        <div className="flex items-center gap-2 text-xs text-zinc-500">
          <button onClick={selectAll} className="text-[var(--primary)] hover:underline">Select all ({filteredCount})</button>
