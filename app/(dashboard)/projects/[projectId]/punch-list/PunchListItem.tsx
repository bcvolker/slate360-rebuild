  ...
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900 shadow-sm overflow-hidden transition-all">
      <button onClick={onToggle} className="flex w-full items-center gap-3 p-4 text-left hover:bg-zinc-800/50 transition">
        ...
        {item.assignee && <span className="hidden md:inline-flex shrink-0 items-center gap-1 rounded-full bg-zinc-800 px-2 py-0.5 text-[10px] font-semibold text-zinc-400"><User size={9} /> {item.assignee}</span>}
        {item.location_area && <span className="hidden lg:inline-flex shrink-0 items-center gap-1 rounded-full bg-zinc-800 px-2 py-0.5 text-[10px] font-semibold text-zinc-400"><MapPin size={9} /> {item.location_area}</span>}
      ...
      {isExpanded && (
        <div className="border-t border-zinc-800 bg-zinc-800/30 p-4 space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-2 pt-2">
            ...
            {item.status === "Closed" && <button onClick={() => onQuickStatus(item, "Open")} className="inline-flex items-center gap-1 rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-1.5 text-xs font-semibold text-zinc-300 hover:bg-zinc-700 transition">Reopen</button>}
            <button onClick={() => onEdit(item)} className="inline-flex items-center gap-1 rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-1.5 text-xs font-semibold text-zinc-300 hover:bg-zinc-700 transition">Edit</button>
            <button onClick={() => onHistory(item)} className="inline-flex items-center gap-1 rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-1.5 text-xs font-semibold text-zinc-400 hover:bg-zinc-700 transition"><History size={12} /> History</button>
            <button onClick={() => onDelete(item.id)} className="inline-flex items-center gap-1 rounded-lg border border-red-900/40 bg-zinc-800 px-3 py-1.5 text-xs font-semibold text-red-400 hover:bg-red-950/30 transition ml-auto"><Trash2 size={12} /> Delete</button>
          </div>
        </div>
      )}
    </div>
