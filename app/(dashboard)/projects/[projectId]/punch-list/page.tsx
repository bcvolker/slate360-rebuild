...
import ChangeHistory, { buildBaseHistory } from "@/components/projects/ChangeHistory";
...
export default function PunchListPage() {
	...
	const filtered = useMemo(() => {
		...
		if (search) {
			...
			list = list.filter((item) => item.title.toLowerCase().includes(q) || item.description?.toLowerCase().includes(q) || item.assignee?.toLowerCase().includes(q) || item.location_area?.toLowerCase().includes(q) || String(item.number).includes(q));
		}
	...
	const stats = useMemo(() => ({
		...
		review: items.filter((item) => item.status === "Ready for Review").length,
	...
	const exportCSV = () => {
		...
		const rows = filtered.map((item) => [String(item.number), item.title, item.description ?? "", item.status, item.priority, item.assignee ?? "", item.location_area ?? "", item.trade_category ?? "", item.due_date ?? "", new Date(item.created_at).toLocaleDateString()]);
		const csv = [header, ...rows].map((row) => row.map((cell) => `"${cell.replace(/"/g, '""')}"`).join(",")).join("\n");
		const blob = new Blob([csv], { type: "text/csv" });
		const url = URL.createObjectURL(blob);
		const anchor = document.createElement("a");
		anchor.href = url;
		anchor.download = `punch-list-${new Date().toISOString().split("T")[0]}.csv`;
		anchor.click();
		...
		<section className="space-y-6">
			...
			<div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
				{([
					...
					<div key={stat.label} className="rounded-xl border border-zinc-800 bg-zinc-900 p-4 shadow-sm">
