...
export default async function MyWorkPage() {
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-4 px-4 py-5 sm:px-6 lg:px-8 lg:py-8">
      <header>
        <p className="text-[11px] font-black uppercase tracking-[0.18em] text-[var(--graphite-primary)]">My Work</p>
      ...
      {assignments.length > 0 ? (
        <section className="space-y-3">
          {assignments.map((a) => (
            <Link key={a.id} href={`/site-walk/walks/${a.session_id}`} className="flex items-center justify-between gap-4 rounded-3xl border border-white/10 bg-white/5 p-4 backdrop-blur-sm transition hover:border-[color-mix(in_srgb,var(--graphite-primary)_40%,transparent)] hover:bg-white/10">
...
function statusColor(s: AssignmentStatus) {
  return s === "acknowledged" ? "bg-white/[0.05] text-[var(--graphite-muted)] border border-white/10" : s === "in_progress" ? "bg-yellow-700/60 text-yellow-200" : "bg-slate-700 text-slate-300";
}
