  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-zinc-800 bg-card">
      <table className="w-full text-left text-xs">
        <thead>
          <tr className="border-b border-zinc-800 bg-card/50 text-[10px] font-semibold uppercase tracking-wide text-zinc-500">
        ...
        <tbody className="divide-y divide-zinc-800/50">
          {files.map((file) => {
            ...
            return (
              <tr key={file.id} className="hover:bg-card/50 cursor-pointer" onClick={() => onSelect(file)}>
                ...
                <td className="px-4 py-2.5 text-right">
                  <button onClick={(e) => { e.stopPropagation(); if (urlMap[file.id]) { const a = document.createElement("a"); a.href = urlMap[file.id]; a.download = file.name; a.target = "_blank"; a.click(); } }} className="rounded-md p-1 text-zinc-500 hover:bg-card hover:text-zinc-300" title="Download"><Download size={14} /></button>
                </td>
              </tr>
            );
          })}
