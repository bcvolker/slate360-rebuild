    ...
    <section className="space-y-4">
      ...
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex items-center gap-2 text-xs text-zinc-500">
          ...
          <span className="ml-1 rounded-md bg-[#FF4D00]/10 px-2 py-0.5 text-[10px] font-semibold text-[#FF4D00] ring-1 ring-[#FF4D00]/20">Saved to /Drawings/</span>
        </div>
      </header>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search drawings…" className="rounded-lg border border-zinc-700 bg-zinc-900 py-1.5 pl-8 pr-3 text-xs text-zinc-200 placeholder:text-zinc-600 focus:border-[#FF4D00] focus:outline-none focus:ring-1 focus:ring-[#FF4D00]/30 w-56" />
            </div>
            <select value={setFilter} onChange={(e) => setSetFilter(e.target.value)} className="rounded-lg border border-zinc-700 bg-zinc-900 px-2.5 py-1.5 text-xs text-zinc-300 focus:border-[#FF4D00] focus:outline-none">
              {DRAWING_SETS.map((s) => <option key={s} value={s}>{s}{s !== "All Sets" && setCounts[s] ? ` (${setCounts[s]})` : ""}</option>)}
            </select>
          </div>
          <div className="flex items-center gap-1">
            <button onClick={() => setViewMode("grid")} className={`rounded-md px-2.5 py-1 text-xs font-semibold transition ${viewMode === "grid" ? "bg-[#FF4D00] text-white" : "border border-zinc-700 bg-zinc-900 text-zinc-400 hover:bg-zinc-800"}`}>Grid</button>
            <button onClick={() => setViewMode("list")} className={`rounded-md px-2.5 py-1 text-xs font-semibold transition ${viewMode === "list" ? "bg-[#FF4D00] text-white" : "border border-zinc-700 bg-zinc-900 text-zinc-400 hover:bg-zinc-800"}`}>List</button>
