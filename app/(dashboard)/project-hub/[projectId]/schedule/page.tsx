  ...
  return (
    <section className="space-y-5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="inline-flex rounded-lg border border-zinc-700 bg-zinc-900 overflow-hidden">
            <button onClick={() => setViewMode("gantt")} className={`px-3 py-2 text-xs font-semibold transition ${viewMode === "gantt" ? "bg-[#D4AF37] text-white" : "text-zinc-400 hover:bg-zinc-800"}`}>Gantt</button>
            <button onClick={() => setViewMode("table")} className={`px-3 py-2 text-xs font-semibold transition ${viewMode === "table" ? "bg-[#D4AF37] text-white" : "text-zinc-400 hover:bg-zinc-800"}`}>Table</button>
          </div>
          {viewMode === "gantt" && (
          ...
          <button onClick={() => { setForm({ ...EMPTY_FORM, assignedTo: profile.contractorName }); setEditingId(null); setShowCreate(true); }} className="inline-flex items-center gap-1.5 rounded-lg bg-[#D4AF37] px-4 py-2 text-sm font-semibold text-white hover:bg-[#E64500] transition"><Plus size={15} /> Add Task</button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-4 text-[10px] font-semibold text-zinc-500">
        ...
        <span className="flex items-center gap-1.5"><span className="w-0.5 h-3 inline-block bg-[#D4AF37]" />Today</span>
