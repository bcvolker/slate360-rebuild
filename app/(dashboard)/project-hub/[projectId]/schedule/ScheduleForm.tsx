...
const inp = "w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 text-sm text-zinc-200 outline-none focus:border-[#F59E0B] focus:ring-1 focus:ring-[#F59E0B]/30 placeholder:text-zinc-500";
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-black/40" onClick={onClose}>
      <div className="h-full w-full max-w-lg overflow-y-auto bg-zinc-900 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-5 p-6">
          ...
          <div>
            <label className={lbl}>% Complete — <span className="text-[#F59E0B]">{form.percentComplete}%</span></label>
            <input type="range" min="0" max="100" step="5" value={form.percentComplete} onChange={(e) => u({ percentComplete: e.target.value })} className="w-full accent-[#F59E0B]" />
          </div>
          ...
          <div className="flex items-center gap-3"><input type="checkbox" id="ms" checked={form.isMilestone} onChange={(e) => u({ isMilestone: e.target.checked })} className="rounded border-zinc-600 accent-[#F59E0B]" /><label htmlFor="ms" className="flex items-center gap-1.5 text-sm font-semibold text-zinc-300"><Flag size={13} className="text-purple-400" /> Mark as Milestone</label></div>
          ...
          <div className="flex items-center gap-3 pt-2">
            <button onClick={onSubmit} disabled={saving || !form.name.trim()} className="inline-flex flex-1 items-center justify-center gap-2 rounded-lg bg-[#F59E0B] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#E64500] disabled:opacity-50 transition">{saving && <Loader2 size={14} className="animate-spin" />}{editingId ? "Update Task" : "Add Task"}</button>
