            ...
            <div style={{ width: totalDays * dayW, minWidth: "100%" }}>
              ...
              <div className="sticky top-8 z-10 bg-zinc-900 border-b border-zinc-800 flex" style={{ height: 32 }}>
                {dayNumbers.map((d, i) => {
                  ...
                  return <div key={i} className={`flex items-center justify-center shrink-0 border-r border-zinc-800/50 text-[8px] font-semibold ${isToday ? "bg-[#F59E0B]/10 text-[#F59E0B] font-black" : isWE ? "bg-zinc-800/40 text-zinc-600" : "text-zinc-500"}`} style={{ width: dayW, minWidth: dayW }} title={d.toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" })}>{dayW >= 24 ? d.getDate() : ""}</div>;
                })}
              </div>
              <div className="relative">
                ...
                {todayOffsetPx >= 0 && todayOffsetPx <= totalDays * dayW && (
                  <div className="absolute top-0 bottom-0 w-0.5 bg-[#F59E0B] z-20 pointer-events-none" style={{ left: todayOffsetPx }}><div className="absolute -top-0 -left-1.5 w-3 h-3 rounded-full bg-[#F59E0B]" /></div>
                )}
