...
export default function ScheduleTable({ rows, loading, onEdit, onHistory, onDelete }: Props) {
  if (loading) return <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-8 text-center text-sm text-zinc-400"><Loader2 size={16} className="mr-2 inline animate-spin" /> Loading...</div>;
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {rows.map((row) => {
              ...
              return (
                <tr key={row.id} className="border-t border-zinc-800 hover:bg-zinc-800/50 transition">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="h-1.5 w-16 rounded-full bg-zinc-800 overflow-hidden"><div className={`h-full rounded-full ${colors.bar}`} style={{ width: `${pct}%` }} /></div>
                  ...
                  <td className="px-4 py-3 text-center">
                    <div className="inline-flex items-center gap-1">
                      <button onClick={() => onEdit(row)} className="rounded-md p-1.5 text-zinc-500 hover:bg-zinc-800 hover:text-zinc-300"><Pencil size={13} /></button>
                      <button onClick={() => onHistory(row)} className="rounded-md p-1.5 text-zinc-500 hover:bg-zinc-800 hover:text-zinc-300" title="History"><History size={13} /></button>
