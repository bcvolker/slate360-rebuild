...
export default function ProjectRFIsPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const payload = (await res.json().catch(() => ({}))) as { rfis?: RfiRow[]; error?: string };
      if (!res.ok) throw new Error(payload.error ?? "Failed to load RFIs");
      setRows(Array.isArray(payload.rfis) ? payload.rfis : []);
    } catch (e) {
  ...
  const sendToClient = async (rfiId: string) => {
    ...
    try {
      const res = await fetch(`/api/projects/${projectId}/external-links`, {
        ...
        body: JSON.stringify({ targetType: "RFI", targetId: rfiId, expiresInDays: 14 }),
      });
  ...
  const onSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(payload.error ?? "Failed to create RFI");
    ...
    <section className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <h2 className="text-lg font-black text-gray-900">RFIs</h2>
        </div>
        <button
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        {loading ? (
          <div className="p-6 text-sm text-gray-500">
            <Loader2 size={16} className="mr-2 inline animate-spin" /> Loading RFIs…
          </div>
        ) : rows.length === 0 ? (
          <div className="p-6 text-sm text-gray-500">No RFIs yet.</div>
        ) : (
        ...
        <div className="fixed inset-0 z-50" onClick={() => setOpen(false)}>
          ...
          >
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-base font-black text-gray-900">Create RFI</h3>
