    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-lg bg-card border-l border-zinc-800 shadow-xl overflow-y-auto animate-in slide-in-from-right duration-300">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-zinc-800 bg-card/95 backdrop-blur px-6 py-4">
          <h2 className="text-lg font-bold text-foreground">{editingId ? "Edit RFI" : "New RFI"}</h2>
        ...
        <div className="space-y-4 p-6">
          ...
          {!editingId && (
            <div>
              ...
              <input type="file" onChange={(e) => setAttachment(e.target.files?.[0] ?? null)} className="w-full rounded-xl border border-zinc-700 bg-card px-3.5 py-2.5 text-sm text-zinc-400 file:mr-3 file:rounded-lg file:border-0 file:bg-zinc-700 file:px-3 file:py-1 file:text-xs file:font-semibold file:text-zinc-300" />
            </div>
          )}
        </div>
        <div className="sticky bottom-0 border-t border-zinc-800 bg-card/95 backdrop-blur px-6 py-4 flex items-center gap-3">
          <button disabled={saving || !form.subject.trim() || !form.question.trim()} onClick={onSubmit} className="flex-1 rounded-xl bg-[#3B82F6] px-4 py-2.5 text-sm font-bold text-foreground hover:bg-[#1d4ed8] transition disabled:opacity-40">{saving ? "Saving…" : editingId ? "Update RFI" : "Create RFI"}</button>
