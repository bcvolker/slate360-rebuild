      ...
      <table className="w-full text-left text-xs">
        ...
        <tbody className="divide-y divide-zinc-800/50">
          {files.map((file, idx) => {
            ...
            return (
              <tr key={file.id} className={`hover:bg-zinc-800/50 ${selected ? "bg-[#FF4D00]/5" : ""}`}>
                ...
                <td className="px-3 py-2">
                  <button onClick={() => onOpenLightbox(idx)} className="font-semibold text-zinc-200 hover:text-[#FF4D00]">{file.name}</button>
                </td>
                <td className="px-3 py-2">
