...
import { useViewPrefs } from "@/components/project-hub/ViewCustomizer";
...
export default function ProjectPhotosPage() {
  ...
  const handleDownloadSelected = () => {
    selectedIds.forEach((id) => { const url = urlMap[id]; const f = files.find((x) => x.id === id); if (url && f) { const a = document.createElement("a"); a.href = url; a.download = f.name; a.target = "_blank"; a.click(); } });
    setToast(`Downloading ${selectedIds.size} photo${selectedIds.size !== 1 ? "s" : ""}...`);
  };

  const onGenerateReport = async () => {
    if (!projectId || generating) return;
    setGenerating(true); setToast("Generating photo report...");
    try {
    ...
    } catch (error) { setToast(error instanceof Error ? error.message : "Failed to generate photo report"); }
    ...
    <section className="space-y-4">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h2 className="text-lg font-black text-foreground">Photo Log</h2>
        ...
        <button onClick={onGenerateReport} disabled={generating}
          className="inline-flex items-center gap-2 rounded-lg bg-[#3B82F6] px-4 py-2 text-sm font-semibold text-foreground transition hover:bg-[#1D4ED8] disabled:opacity-60">
      ...
      {!loading && files.length > 0 && (
        <PhotosToolbar
          ...
          viewPrefs={viewPrefs} setViewPrefs={setViewPrefs} projectId={projectId}
        />
      )}

      {/* Content */}
      {loading ? (
        <div className="rounded-2xl border border-zinc-800 bg-card p-8 text-sm text-zinc-400">
        ...
        <div className="rounded-2xl border border-dashed border-zinc-700 bg-card p-8 text-center text-sm text-zinc-400">No photos match your search or filter.</div>
      ) : viewMode === "list" ? (
