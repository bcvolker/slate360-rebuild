    ...
    return (
      <div className="columns-1 gap-4 sm:columns-2 lg:columns-3 xl:columns-4">
        {files.map((file, idx) => {
          ...
          return (
            <article key={file.id} className={`group relative mb-4 break-inside-avoid overflow-hidden rounded-xl border transition hover:shadow-md ${selected ? "border-[#F59E0B] ring-2 ring-[#F59E0B]/20" : "border-zinc-800"}`}>
              <button onClick={(e) => { e.stopPropagation(); toggleSelect(file.id); }}
                className={`absolute left-2 top-2 z-10 flex h-5 w-5 items-center justify-center rounded border text-white transition ${selected ? "border-[#F59E0B] bg-[#F59E0B]" : "border-white/70 bg-black/20 opacity-0 group-hover:opacity-100"}`}>
  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
      {files.map((file, idx) => {
        ...
        const category = guessCategory(file.name);
        return (
          <div key={file.id} className={`group relative overflow-hidden rounded-xl border transition hover:shadow-md ${selected ? "border-[#F59E0B] ring-2 ring-[#F59E0B]/20" : "border-zinc-800"}`}>
            <button onClick={(e) => { e.stopPropagation(); toggleSelect(file.id); }}
              className={`absolute left-2 top-2 z-10 flex h-5 w-5 items-center justify-center rounded border text-white transition ${selected ? "border-[#F59E0B] bg-[#F59E0B]" : "border-white/70 bg-black/20 opacity-0 group-hover:opacity-100"}`}>
              {selected && <span className="text-xs">✓</span>}
            </button>
