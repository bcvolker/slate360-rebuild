          ...
          <div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <select value={reportType} onChange={(e) => setReportType(e.target.value)} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm outline-none focus:border-[#3B82F6]">
                  {REPORT_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
                </select>
              </div>
              <div>
                <label className="mb-1 block text-xs font-bold text-gray-700">Custom Title (optional)</label>
                <input value={reportTitle} onChange={(e) => setReportTitle(e.target.value)} placeholder={`${projectName ? projectName + " — " : ""}${reportType} Report`} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm outline-none focus:border-[#3B82F6]" />
              </div>
              <div>
                ...
                <div className="space-y-2">
                  {SECTIONS_OPTIONS.map((s) => (
                    <label key={s.id} className="flex items-center gap-2.5 cursor-pointer">
                      <input type="checkbox" checked={reportSections.includes(s.id)} onChange={(e) => setReportSections(e.target.checked ? [...reportSections, s.id] : reportSections.filter((r) => r !== s.id))} className="h-4 w-4 rounded border-gray-300 accent-[#3B82F6]" />
