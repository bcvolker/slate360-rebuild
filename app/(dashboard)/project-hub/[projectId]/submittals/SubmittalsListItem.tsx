  ...
  return (
    <div className="rounded-xl border border-zinc-800 bg-card shadow-sm overflow-hidden transition-all">
      <button onClick={onToggle} className="flex w-full items-center gap-3 p-4 text-left hover:bg-card/50 transition">
        <span className={`inline-flex shrink-0 rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${STATUS_COLORS[sub.status] ?? "bg-card text-zinc-400"}`}>{sub.status}</span>
        <div className="flex-1 min-w-0">
          <span className="truncate text-sm font-semibold text-foreground">{sub.title}</span>
        ...
        {(sub.revision_number ?? 0) > 0 && <span className="hidden sm:inline-flex shrink-0 rounded-full bg-card px-2 py-0.5 text-[10px] font-bold text-zinc-400">Rev {sub.revision_number}</span>}
        {sub.responsible_contractor && <span className="hidden md:inline-flex shrink-0 items-center gap-1 rounded-full bg-card px-2 py-0.5 text-[10px] font-semibold text-zinc-400"><User size={9} /> {sub.responsible_contractor}</span>}
      ...
      {expanded && (
        <div className="border-t border-zinc-800 bg-card/30 p-4 space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-2 pt-2">
            ...
            <button onClick={onHistory} className={`${btn} border-zinc-700 bg-card text-zinc-400 hover:bg-zinc-700`}><History size={12} /> History</button>
            <button onClick={onDelete} className={`${btn} border-red-900/50 bg-card text-red-400 hover:bg-red-950/30 ml-auto`}><Trash2 size={12} /> Delete</button>
          </div>
        </div>
      )}
    </div>
