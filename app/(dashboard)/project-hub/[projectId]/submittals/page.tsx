    ...
    <section className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h2 className="text-xl font-black text-foreground">Submittals</h2>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => { setForm(EMPTY_FORM); setEditingId(null); setAttachment(null); setShowCreate(true); }} className="inline-flex items-center gap-1.5 rounded-lg bg-[#3B82F6] px-4 py-2 text-sm font-semibold text-foreground hover:bg-[#1D4ED8] transition"><Plus size={15} /> New Submittal</button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search submittals..." className="w-full rounded-lg border border-zinc-700 bg-card py-2 pl-9 pr-3 text-sm text-zinc-200 placeholder:text-zinc-500 outline-none focus:border-[#3B82F6] focus:ring-1 focus:ring-[#3B82F6]/30" />
        </div>
      ...
      {showFilters && (
        <div className="flex flex-wrap gap-3 rounded-xl border border-zinc-800 bg-card p-4">
          <div>
            ...
            <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="rounded-lg border border-zinc-700 bg-card px-3 py-1.5 text-sm text-zinc-300 outline-none">
          ...
          <div>
            ...
            <select value={filterDocType} onChange={(e) => setFilterDocType(e.target.value)} className="rounded-lg border border-zinc-700 bg-card px-3 py-1.5 text-sm text-zinc-300 outline-none">
      ...
      {/* List */}
      {loading ? (
        <div className="rounded-2xl border border-zinc-800 bg-card p-8 text-center text-sm text-zinc-400"><Loader2 size={16} className="mr-2 inline animate-spin" /> Loading submittals...</div>
      ) : rows.length === 0 ? (
        ...
        <div className="rounded-2xl border border-zinc-800 bg-card p-8 text-center text-sm text-zinc-400">No submittals match your filters.</div>
      ) : (
