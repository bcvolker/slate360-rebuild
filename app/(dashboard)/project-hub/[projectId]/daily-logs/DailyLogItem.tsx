        ...
        <div className="border-t border-zinc-800 bg-card/30 p-4 space-y-4">
          ...
          {log.equipment && log.equipment.length > 0 && (
            <div><p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500 mb-2">Equipment on Site</p>
              <div className="flex flex-wrap gap-2">{log.equipment.map((eq, i) => (<span key={i} className="inline-flex rounded-full bg-card px-2.5 py-1 text-xs font-semibold text-zinc-300">{eq}</span>))}</div>
            </div>
          )}
          ...
          <div className="flex items-center gap-2 pt-2">
            <button onClick={() => onEdit(log)} className="inline-flex items-center gap-1 rounded-lg border border-zinc-700 bg-card px-3 py-1.5 text-xs font-semibold text-zinc-300 hover:bg-zinc-700 transition">Edit</button>
            <button onClick={() => onHistory(log)} className="inline-flex items-center gap-1 rounded-lg border border-zinc-700 bg-card px-3 py-1.5 text-xs font-semibold text-zinc-400 hover:bg-zinc-700 transition"><History size={12} /> History</button>
            <button onClick={() => onDelete(log.id)} className="inline-flex items-center gap-1 rounded-lg border border-red-900/40 bg-card px-3 py-1.5 text-xs font-semibold text-red-400 hover:bg-red-950/30 transition ml-auto"><Trash2 size={12} /> Delete</button>
          </div>
        </div>
      )}
    </div>
