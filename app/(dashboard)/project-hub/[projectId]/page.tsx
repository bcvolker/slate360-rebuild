...
export default async function ProjectHubProjectPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  const stats = [
    { label: "Open RFIs", value: openRfis, sub: `${rfisData.length} total`, href: `/project-hub/${projectId}/rfis`, color: "#F59E0B", Icon: ClipboardList },
    ...
    { label: "Schedule Tasks", value: pendingTasks, sub: "active tasks", href: `/project-hub/${projectId}/schedule`, color: "#F59E0B", Icon: CalendarCheck2 },
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-black text-white flex items-center gap-2">
                <CalendarCheck2 size={14} className="text-[#F59E0B]" /> Upcoming Schedule
              </h3>
              <Link href={`/project-hub/${projectId}/schedule`} className="text-[10px] font-bold text-[#F59E0B] hover:underline">Open Schedule →</Link>
            </div>
            {nextTasks.length === 0 ? (
              ...
              <ul className="space-y-2">
                {nextTasks.map((t) => (
                  <li key={t.id} className="flex items-start gap-3 p-2.5 rounded-xl bg-zinc-800 border border-zinc-700">
                    <div className="w-2 h-2 rounded-full mt-1.5 shrink-0" style={{ backgroundColor: t.status === "In Progress" ? "#F59E0B" : "#d1d5db" }} />
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
            ...
            <div className="grid grid-cols-3 gap-2">
              {toolLinks.map(({ label, href, Icon }) => (
                <Link key={label} href={href} className="flex flex-col items-center gap-1.5 p-2.5 rounded-xl border border-zinc-800 bg-zinc-800/50 hover:bg-[#F59E0B]/10 hover:border-[#F59E0B]/30 transition-all text-center">
