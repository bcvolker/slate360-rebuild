...
export default function BudgetForm({ form, setForm, editingId, saving, rows, totals, revisedBudget, variance, pctSpent, categoryBreakdown, onSubmit, onDelete, onCancelEdit }: Props) {
  return (
    <div className="rounded-2xl border border-zinc-800 bg-card shadow-sm flex flex-col">
      <div className="border-b border-zinc-800 px-5 py-4">
        <h3 className="text-sm font-black text-foreground">{editingId ? "Edit Line Item" : "Add Line Item"}</h3>
        <p className="text-[10px] text-zinc-500 mt-0.5">{editingId ? "Update fields below" : "Fill in the details and click Add"}</p>
      </div>
      <div className="flex-1 overflow-y-auto p-5 space-y-4">
        {rows.length > 0 && (
          <div className="rounded-xl border border-zinc-800 bg-card/50 p-3 space-y-2">
            <p className="text-[9px] font-bold uppercase tracking-wider text-zinc-500">Budget Summary</p>
            <div className="flex justify-between text-xs"><span className="text-zinc-400">Original Budget</span><span className="font-bold text-foreground">{fmtCurrency(totals.budget)}</span></div>
            ...
            <div className="flex justify-between text-xs border-t border-zinc-700 pt-1.5"><span className="font-semibold text-zinc-300">Revised Budget</span><span className="font-black text-amber-400">{fmtCurrency(revisedBudget)}</span></div>
            <div className="flex justify-between text-xs"><span className="text-zinc-400">Spent to Date</span><span className="font-bold text-foreground">{fmtCurrency(totals.spent)}</span></div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div><label className="mb-1 block text-[10px] font-bold text-zinc-400">Cost Code *</label><input type="text" value={form.costCode} onChange={(e) => setForm({ ...form, costCode: e.target.value })} placeholder="03-100" className="w-full rounded-lg border border-zinc-700 bg-card px-2 py-1.5 text-xs text-zinc-200 outline-none focus:border-[#3B82F6]" /></div>
          <div><label className="mb-1 block text-[10px] font-bold text-zinc-400">Category</label><select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="w-full rounded-lg border border-zinc-700 bg-card px-2 py-1.5 text-xs text-zinc-200 outline-none"><option value="">Select…</option>{CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}</select></div>
        </div>
        <div><label className="mb-1 block text-[10px] font-bold text-zinc-400">Description</label><input type="text" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Line item description" className="w-full rounded-lg border border-zinc-700 bg-card px-2 py-1.5 text-xs text-zinc-200 outline-none focus:border-[#3B82F6]" /></div>
        <div className="grid grid-cols-2 gap-3">
          <div><label className="mb-1 block text-[10px] font-bold text-zinc-400">Budget ($)</label><input type="number" step="0.01" value={form.budgetAmount} onChange={(e) => setForm({ ...form, budgetAmount: e.target.value })} className="w-full rounded-lg border border-zinc-700 bg-card px-2 py-1.5 text-xs text-zinc-200 outline-none focus:border-[#3B82F6]" /></div>
          <div><label className="mb-1 block text-[10px] font-bold text-zinc-400">Spent ($)</label><input type="number" step="0.01" value={form.spentAmount} onChange={(e) => setForm({ ...form, spentAmount: e.target.value })} className="w-full rounded-lg border border-zinc-700 bg-card px-2 py-1.5 text-xs text-zinc-200 outline-none focus:border-[#3B82F6]" /></div>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div><label className="mb-1 block text-[10px] font-bold text-zinc-400">Change Orders ($)</label><input type="number" step="0.01" value={form.changeOrderAmount} onChange={(e) => setForm({ ...form, changeOrderAmount: e.target.value })} className="w-full rounded-lg border border-zinc-700 bg-card px-2 py-1.5 text-xs text-zinc-200 outline-none focus:border-[#3B82F6]" /></div>
          <div><label className="mb-1 block text-[10px] font-bold text-zinc-400">Forecast ($)</label><input type="number" step="0.01" value={form.forecastAmount} onChange={(e) => setForm({ ...form, forecastAmount: e.target.value })} className="w-full rounded-lg border border-zinc-700 bg-card px-2 py-1.5 text-xs text-zinc-200 outline-none focus:border-[#3B82F6]" /></div>
        </div>
        <div><label className="mb-1 block text-[10px] font-bold text-zinc-400">Notes</label><textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} rows={2} placeholder="Notes…" className="w-full rounded-lg border border-zinc-700 bg-card px-2 py-1.5 text-xs text-zinc-200 outline-none resize-none" /></div>
      </div>
      <div className="border-t border-zinc-800 p-4 space-y-2">
        <button onClick={onSubmit} disabled={saving || !form.costCode.trim()} className="w-full inline-flex items-center justify-center gap-2 rounded-lg bg-[#3B82F6] px-4 py-2.5 text-sm font-semibold text-foreground hover:bg-[#1D4ED8] disabled:opacity-50 transition">{saving ? <Loader2 size={14} className="animate-spin" /> : <Plus size={14} />}{editingId ? "Update Line Item" : "Add Line Item"}</button>
        {editingId && (
          <div className="flex gap-2">
            <button onClick={onCancelEdit} className="flex-1 rounded-lg border border-zinc-700 px-3 py-2 text-xs font-semibold text-zinc-300 hover:bg-card">Cancel</button>
