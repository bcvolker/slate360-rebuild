    ...
    <div className="rounded-2xl border border-zinc-800 bg-zinc-900 shadow-sm flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div><label className="mb-1 block text-[10px] font-bold text-zinc-400">Cost Code *</label><input type="text" value={form.costCode} onChange={(e) => setForm({ ...form, costCode: e.target.value })} placeholder="03-100" className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-2 py-1.5 text-xs text-zinc-200 outline-none focus:border-[#FF4D00]" /></div>
        ...
        <div><label className="mb-1 block text-[10px] font-bold text-zinc-400">Description</label><input type="text" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Line item description" className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-2 py-1.5 text-xs text-zinc-200 outline-none focus:border-[#FF4D00]" /></div>
        <div className="grid grid-cols-2 gap-3">
          <div><label className="mb-1 block text-[10px] font-bold text-zinc-400">Budget ($)</label><input type="number" step="0.01" value={form.budgetAmount} onChange={(e) => setForm({ ...form, budgetAmount: e.target.value })} className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-2 py-1.5 text-xs text-zinc-200 outline-none focus:border-[#FF4D00]" /></div>
          <div><label className="mb-1 block text-[10px] font-bold text-zinc-400">Spent ($)</label><input type="number" step="0.01" value={form.spentAmount} onChange={(e) => setForm({ ...form, spentAmount: e.target.value })} className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-2 py-1.5 text-xs text-zinc-200 outline-none focus:border-[#FF4D00]" /></div>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div><label className="mb-1 block text-[10px] font-bold text-zinc-400">Change Orders ($)</label><input type="number" step="0.01" value={form.changeOrderAmount} onChange={(e) => setForm({ ...form, changeOrderAmount: e.target.value })} className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-2 py-1.5 text-xs text-zinc-200 outline-none focus:border-[#FF4D00]" /></div>
          <div><label className="mb-1 block text-[10px] font-bold text-zinc-400">Forecast ($)</label><input type="number" step="0.01" value={form.forecastAmount} onChange={(e) => setForm({ ...form, forecastAmount: e.target.value })} className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-2 py-1.5 text-xs text-zinc-200 outline-none focus:border-[#FF4D00]" /></div>
        </div>
      ...
      <div className="border-t border-zinc-800 p-4 space-y-2">
        <button onClick={onSubmit} disabled={saving || !form.costCode.trim()} className="w-full inline-flex items-center justify-center gap-2 rounded-lg bg-[#FF4D00] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#E64500] disabled:opacity-50 transition">{saving ? <Loader2 size={14} className="animate-spin" /> : <Plus size={14} />}{editingId ? "Update Line Item" : "Add Line Item"}</button>
        {editingId && (
