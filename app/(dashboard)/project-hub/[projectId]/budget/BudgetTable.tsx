      ...
      <table className="w-full text-left text-sm">
        ...
        <tbody>
          {filtered.map((row) => {
            ...
            return (
              <>
                <tr key={row.id} className={`border-t border-zinc-800 transition cursor-pointer ${isEditing ? "bg-[#FF4D00]/5 ring-1 ring-inset ring-[#FF4D00]/20" : "hover:bg-zinc-800/50"}`} onClick={() => setExpandedId(isExpanded ? null : row.id)}>
                  ...
                  <td className="px-4 py-3 text-center" onClick={(e) => e.stopPropagation()}>
                    <div className="inline-flex items-center gap-0.5">
                      <button onClick={() => { onEdit(row); setExpandedId(null); }} className={`rounded p-1 ${isEditing ? "text-[#FF4D00]" : "text-zinc-500 hover:bg-zinc-800 hover:text-zinc-300"}`} title="Edit"><Pencil size={12} /></button>
