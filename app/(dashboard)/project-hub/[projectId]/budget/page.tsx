    ...
    <section className="space-y-5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h2 className="text-xl font-black text-white">Budget & Cost Control{profile.projectName && <span className="ml-2 text-base font-semibold text-zinc-500">— {profile.projectName}</span>}</h2>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        {[
          { label: "Original Budget", value: fmtCurrency(totals.budget), icon: DollarSign, color: "text-white" },
          { label: "Change Orders", value: fmtCurrency(totals.changeOrders), icon: Plus, color: totals.changeOrders > 0 ? "text-amber-400" : "text-white" },
          { label: "Revised Budget", value: fmtCurrency(revisedBudget), icon: DollarSign, color: "text-blue-400" },
          { label: "Spent to Date", value: fmtCurrency(totals.spent), icon: TrendingDown, color: "text-white" },
          ...
          { label: "% Spent", value: `${pctSpent}%`, icon: DollarSign, color: pctSpent > 90 ? "text-red-400" : "text-white" },
        ].map((s) => (
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[180px]">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search cost codes…" className="w-full rounded-lg border border-zinc-700 bg-zinc-900 py-2 pl-8 pr-3 text-sm text-zinc-200 placeholder:text-zinc-600 outline-none focus:border-[#3B82F6]" />
        </div>
        <button onClick={() => setShowFilters(!showFilters)} className={`inline-flex items-center gap-1.5 rounded-lg border px-3 py-2 text-xs font-semibold transition ${showFilters ? "border-[#3B82F6] bg-[#3B82F6]/10 text-[#3B82F6]" : "border-zinc-700 bg-zinc-900 text-zinc-300 hover:bg-zinc-800"}`}><Filter size={13} /> Filters</button>
      </div>
      {showFilters && (
        <div className="flex flex-wrap gap-3 rounded-xl border border-zinc-800 bg-zinc-900 p-4">
          <div><label className="mb-1 block text-[10px] font-bold uppercase tracking-wider text-zinc-500">Category</label><select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)} className="rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-1.5 text-sm text-zinc-200 outline-none"><option value="all">All</option>{CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}</select></div>
        </div>
      )}
      ...
      {showFromScratch && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowFromScratch(false)}>
          <div className="w-full max-w-md rounded-2xl bg-zinc-900 border border-zinc-800 shadow-2xl p-6 space-y-5" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-black text-white">New Budget from Scratch</h3>
              <button onClick={() => setShowFromScratch(false)} className="rounded-lg p-1 text-zinc-400 hover:bg-zinc-800"><X size={18} /></button>
            </div>
            ...
            <div>
              ...
              <input type="text" value={scratchName || profile.projectName} onChange={(e) => setScratchName(e.target.value)} placeholder={profile.projectName || "Enter project name…"} className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 text-sm text-zinc-200 outline-none focus:border-[#3B82F6]" />
            </div>
