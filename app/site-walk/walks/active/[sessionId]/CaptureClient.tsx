...
export default function CaptureClient({ sessionId, title }: { sessionId: string; title: string }) {
  ...
  async function uploadPhoto(file: File) {
    ...
    try {
      ...
      // Linear flow: photo → markup → annotate. Markup is optional but the
      // entry point is the markup screen so the user always has the choice.
      const newId = cj.item?.id as string | undefined;
      if (newId) {
        router.push(`/site-walk/walks/active/${sessionId}/items/${newId}/markup`);
        return;
      }
  ...
  async function saveNote() {
    ...
    try {
      ...
      setShowNote(false);
      if (newId) {
        router.push(`/site-walk/walks/active/${sessionId}/items/${newId}/annotate`);
        return;
      }
