...
export function CaptureModeToggle({ mode, onPlan, onCamera }: CaptureModeToggleProps) {
  return (
    <div className="absolute left-1/2 top-3 z-30 flex -translate-x-1/2 rounded-2xl border border-white/15 bg-slate-950/75 p-1 shadow-2xl backdrop-blur-xl">
      <button type="button" onClick={onPlan} className={`inline-flex h-9 items-center gap-1 rounded-xl px-3 text-[10px] font-black uppercase tracking-[0.1em] ${mode === "plan" ? "bg-[var(--graphite-primary)] text-[var(--graphite-canvas)]" : "text-white/70"}`}>
        <Map className="h-3.5 w-3.5" /> Plan
      </button>
      <button type="button" onClick={onCamera} className={`inline-flex h-9 items-center gap-1 rounded-xl px-3 text-[10px] font-black uppercase tracking-[0.1em] ${mode === "camera" ? "bg-[var(--graphite-primary)] text-[var(--graphite-canvas)]" : "text-white/70"}`}>
