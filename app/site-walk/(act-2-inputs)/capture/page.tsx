...
type Props = {
  searchParams: Promise<{ session?: string; plan?: string; quick?: string; launch?: string }>;
};
...
export default async function SiteWalkCapturePage({ searchParams }: Props) {
  const { session: sessionId, plan, quick, launch } = await searchParams;
  ...
  return <CaptureNoSsrBoundary session={session} showPlanCanvas={showPlanCanvas} autoOpenCamera={quick === "camera"} launchId={launch ?? null} />;
}
