...
| SW-011 | **SHA-256 content hash + chain-of-custody** (evidentiary). **Capture hash DONE** (`content-hash.ts` → `content_sha256` in item metadata). **Audit log DONE (code)** — additive migration `supabase/migrations/20260628120000_evidence_and_sync_foundation.sql` (`evidence_events` append-only + hash-chain, HLC/integrity columns on `site_walk_items`, `entity_tombstones`); `lib/site-walk/evidence-events.ts` `recordEvidenceEvent()` (non-fatal, hash-chained) emits **`captured`** on item create. Migration **APPLIED on prod** (Jun 28). **Server re-verify DONE** — `POST /api/site-walk/items/[id]/verify-hash` refetches the S3 object, recomputes SHA-256, compares to the capture hash, writes `server_sha256`/`hash_verified_at`, and emits `hash_verified`/`hash_mismatch`. **Auto-verify DONE** — `createCaptureItem` now fires the verify-hash endpoint (non-blocking, browser-fired so it runs to completion; skips offline `item-` ids). So the full chain runs automatically: capture → `captured` → auto re-verify → `hash_verified` → `deliverable_created` → **`included_in_deliverable`** (per item, at share) → `deliverable_shared` (share route) → `deliverable_viewed` (recordDeliverableView). **`included_in_deliverable` DONE** — `lib/site-walk/record-inclusion-events.ts` runs in the share route on first-share + `refresh`: extracts item IDs from the deliverable `content`, validates them against `site_walk_items` (org-scoped), dedups via a single `evidence_events` pre-query (so a re-share only attests newly-added items), and emits one event per item carrying the item's `capture_sha256`. Per-item chain of custody proves exactly which captures went into the delivered report. **Certified Evidence export DONE** — `GET /api/site-walk/deliverables/[id]/evidence-export` streams a ZIP bundling each included item's original media (re-hashed at export), its append-only `evidence_events` chain (`evidence/<id>.json`), a machine-readable `MANIFEST.json`, and a **zero-dependency `verify.mjs`** + README. Builder `lib/site-walk/evidence-export.ts` (JSZip, slatedrop/zip caps: ~1.5GB / 1000 files → 413; bounded concurrency 4; bytes fetched once, reused for hash + archive). The verifier validates (1) media SHA-256 vs manifest and (2) chain **linkage** (`prev_hash → event_hash`) — it does NOT re-derive each `event_hash` (jsonb doesn't preserve key order, so re-serialized metadata wouldn't reproduce the server canonical; linkage is the reproducible guarantee). Emits an `evidence_exported` event. **Evidence spine COMPLETE.** UI download affordance = follow-up (desktop deliverable editor). | P1 | Full evidentiary spine shipped: capture hash → re-verify → lifecycle events → per-item inclusion → certified export |
| SW-014 | **Raw-note vs AI-formatted dual storage** + provenance (evidentiary). **DONE** — `/api/site-walk/notes/format` returns `provenance` (model/provider/formatted_at/policy `format_only_no_new_facts`); `useCaptureItems.formatNotesWithAi` preserves the **verbatim raw note** + provenance onto `item.metadata` (`note_raw`/`ai_formatted`/`ai_provenance`) before the AI version overwrites the field — raw is never lost.  **Disclosure badge DONE** — `ViewerMetadata.ai_formatted` carried via `source-items`; the share viewer shows "✦ AI-formatted · original preserved".  **"View original" DONE** — `note_raw` carried into `ViewerMetadata`; the viewer badge expands (native `<details>`) to the verbatim original when present. SW-014 fully complete. | P1 | Dual-storage shipped |
| CONFLICT-001 | Conflict resolution via **HLC**. **Foundation shipped** — `hlc`/`author_node_id`/`conflict_flag` columns live (migration 20260628120000); item PATCH (`/api/site-walk/items/[id]`) accepts a client `hlc`, stamps it, and sets `conflict_flag` when the incoming write isn't strictly newer than the stored one (LWW applies, conflict surfaced). **TODO:** client HLC generation; conflict-review UX (keep mine/theirs/both); append-only events for media. | P2 | HLC server foundation shipped |
