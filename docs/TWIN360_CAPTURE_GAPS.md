...
| REPORT-004 | **Cinematic slideshow rework** — `DeliverableSlideshow.tsx` currently click-throughs EditorBlocks; must (a) render real `ViewerItem[]`, (b) per-stop picture with **info to the LEFT**, (c) horizontal **timeline scrubber** of stops, (d) prev/next + **gentle transitions**, (e) in-slideshow **Q&A/chat** UI, (f) **branding/logo** (slideshow takes no branding props today). Biggest remaining presentation build. | P1 | MISSING — harness is the design target |
