...
| 7 | Complete | `9e571ec` | Plan canvas, long-press pins, and editable markup. | Hotfixed the Site Walk module shell by making `/site-walk` resilient to project-list load failures, replacing direct browser UUID calls with a guarded client session ID fallback, and removing user-facing Act labels from active Site Walk pages. Updated capture UX so mobile is camera-first with a giant Take Photo action while desktop hides the camera-first controls and shows a drag/drop Upload from Device zone. Replaced the plan viewer scaffold with a project-bound plan canvas that loads sheets/pins, supports pan, pinch/zoom controls, long-press draft pins with haptic feedback, quick actions for attaching the next photo/note to `plan_sheet_id` coordinates, and basic vector markup JSON stored through draft pin `markup_data`. Validated changed-file diagnostics, `npm run typecheck`, `npm run build`, `git diff --check`, and the file-size guard, which still reports only pre-existing oversized files. |
