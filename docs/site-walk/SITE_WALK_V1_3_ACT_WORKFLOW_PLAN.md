...
| 10S | Complete | pending commit | Centered attachment panels and contained preview modal. | Centered the new attachment and edit attachment panels inside the capture area so action buttons are no longer cut off by the lower markup controls. Reduced the attached-file preview modal size, added a persistent top-right close button, and added pinch/drag zoom for image previews. |
