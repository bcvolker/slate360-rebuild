...
| 10T | Complete | pending commit | Preview event isolation and attachment fallback. | Further contained marker file previews by shrinking the modal, making the close button a prominent cyan control, and stopping preview pointer/wheel events from bubbling into the capture canvas so pinch gestures zoom the preview instead of the photo behind it. Added an item-level attachment pin resolver so the capture canvas and Attached sheet read persisted `photo_attachment_pins` when metadata is missing or stale. |
