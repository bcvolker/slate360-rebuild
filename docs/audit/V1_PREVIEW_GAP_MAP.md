...
15. **Home 2×2 command grid (2026-05-15):** Removed duplicate SlateDrop/Coordination/Deliverables cards from Home (already in bottom nav). Moved Search from header into command grid. Home now has 4 cards in 2×2 grid: Create Worksite, Walk from Worksite, Quick Walk, Search. Two-zone grid retained: `grid-rows-[1fr_minmax(285px,305px)]`. Landscape: `landscape:grid-cols-2 landscape:grid-rows-[1fr]` side-by-side layout.
