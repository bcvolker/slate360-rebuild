...
15. **Home CSS grid layout (2026-05-15):** Root cause was `flex h-full flex-col` inheriting excess height. Fixed with `grid grid-rows-[auto_auto_auto_minmax(285px,305px)] content-start`. Row 4 is clamped at 285–305px (~3.3–3.6 rows). `content-start` pushes leftover space below the grid, not between rows. No 1fr, no flex-1, no mt-auto. Panel scrolls internally. Bottom fade retained.
