...
15. **Home layout contract (2026-05-15):** Fixed-height mobile layout. No flex-1 or mt-auto on Work panel. Label text-[14px] amber-400. Primary actions min-h-[108px] with size-[30px] icons. Core tools h-[72px] with size-6 icons, text-[14px]. Work panel h-[300px] min-h-[285px] max-h-[305px] shrink-0 (no flex growth). Walk rows min-h-[72px]. ~3.5 visible rows. mt-5 tools→panel gap, mb-3 panel→nav gap. Bottom fade retained.
