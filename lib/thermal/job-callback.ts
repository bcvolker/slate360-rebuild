...
import { computePartialFailure } from "@/lib/thermal/job-partial-failure";
import {
  applyAlignResults,
  applyAnalyzeResults,
  applyExtractResults,
  insertReportRow,
} from "@/lib/thermal/job-result-appliers";
...
export async function handleThermalJobCallback(
  ...
  const { data: job, error: jobError } = await admin
    ...
    .select("id, session_id, org_id, status, job_type, created_by, input_capture_ids, dedupe_key")
    .eq("id", body.jobId)
    .maybeSingle();
  ...
  const resultCaptureIds = new Set<string>([
    ...extractResults.map((r) => r.captureId),
    ...analyzeResults.map((r) => r.captureId),
    ...alignResults.map((r) => r.captureId),
  ]);
  const { failedCaptureIds, isPartial, errorLog } = computePartialFailure(
    (job.input_capture_ids as string[] | null) ?? [],
    resultCaptureIds,
    extractResults.length > 0 || analyzeResults.length > 0 || alignResults.length > 0,
  );
  ...
  await admin
    ...
    .update({
      ...
      partial: isPartial,
      failed_capture_ids: failedCaptureIds,
      error_log: errorLog,
    })
    .eq("id", job.id);
