...
import { classifySource, ingestHint, isBrowserPanorama, isErp } from "./source-class";
...
describe("ingest source class", () => {
  it("never treats raw Insta360 as a browser panorama", () => {
    ...
    expect(ingestHint(kind)).toMatch(/Insta360 Studio/);
  });
  ...
  it("classifies iPhone depth, lidar sidecars, and documents", () => {
    expect(classifySource({ fileName: "scan.s360depth" })).toBe("IPHONE_RGBD");
