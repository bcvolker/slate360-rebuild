      ...
      <div className="h-2.5 rounded-full bg-white/[0.04] overflow-hidden">
        <div
          className={`h-full rounded-full transition-all duration-500 ${
            isCritical ? "bg-red-500" : isHigh ? "bg-amber-500" : "bg-amber-500"
          }`}
          style={{ width: `${pct}%` }}
        />
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-app bg-app-card p-6 space-y-5">
        <h3 className="text-sm font-bold text-zinc-100 flex items-center gap-2">
          <Zap size={16} className="text-amber-500" /> Usage Overview
          <span className="ml-auto text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-500">
            {tierLabel}
          </span>
        </h3>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div key={item.label} className="rounded-xl bg-white/[0.04] p-3 text-center">
              <item.icon size={18} className="text-amber-500 mx-auto mb-1.5" />
      ...
      <div className="rounded-2xl border border-app bg-app-card p-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="text-lg font-bold text-amber-500">{(billing?.totalCreditsBalance ?? 0).toLocaleString()}</p>
          </div>
          <div>
        ...
        {isAdmin && !isCeo && (
          <button
            ...
            className="mt-4 inline-flex items-center gap-2 rounded-xl bg-amber-500 px-4 py-2 text-xs font-semibold text-zinc-950 hover:bg-amber-500/80 transition-colors"
          >
