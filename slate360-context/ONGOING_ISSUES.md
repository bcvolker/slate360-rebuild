...
| BUG-018 | LocationMap | **DrawingManager deprecation — `LocationMap.tsx` confirmed source (not WizardLocationPicker):** `components/dashboard/LocationMap.tsx` still loads `useMapsLibrary("drawing")` and `DrawingManager`, but migration is now in progress. **Completed:** marker flow (tranche 1) and line/arrow/polygon flows (tranche 2) moved to native `google.maps` listener-based drawing. **Remaining:** rectangle/circle native parity and full removal of `drawing` library from `LocationMap.tsx` + APIProvider libraries. **Migration deadline: May 2026.** See `slate360-context/BACKEND.md §9b` for the migration approach. | High | 🟡 In Progress — final tranche pending |
...
| `Drawing library functionality in the Maps JavaScript API is deprecated` | `LocationMap.tsx` (NOT WizardLocationPicker) | **Root cause confirmed:** `components/dashboard/LocationMap.tsx` still includes `useMapsLibrary("drawing")` and `"drawing"` in APIProvider libraries. **Progress:** marker + line/arrow/polygon flows migrated to native map listeners; rectangle/circle still rely on DrawingManager. This warning will become a **break** in May 2026 when Google removes the library. | 🟡 BUG-018 in progress (May 2026 deadline) | Complete rectangle/circle migration, then remove DrawingManager + `"drawing"` library wiring from `LocationMap.tsx`. |
...
Progress update (Mar 4 2026): extracted `SlateDropContextMenu.tsx`, `SlateDropActionModals.tsx`, `SlateDropSharePreviewModals.tsx`, `SlateDropFileArea.tsx`, `SlateDropSidebar.tsx`, `SlateDropTopBar.tsx`, `SlateDropToolbar.tsx`, and `SlateDropNotificationsOverlay.tsx`, plus `useSlateDropFiles`, `useSlateDropUiState`, `useSlateDropTransferActions`, `useSlateDropMutationActions`, `useSlateDropInteractionHandlers`, `useSlateDropUploadActions`, `useSlateDropPreviewUrl`, and shared helper module `lib/slatedrop/client-utils.ts`; optional shell polish is complete, Project Hub Slice C is complete, and Dashboard decomposition now includes extracted widget shell/views: `DashboardWidgetGrid.tsx`, `DashboardWidgetPopout.tsx`, `DashboardDataUsageWidget.tsx`, `DashboardProcessingWidget.tsx`, `DashboardFinancialWidget.tsx`, `DashboardCalendarWidget.tsx`, `DashboardWeatherWidget.tsx`, `DashboardContinueWidget.tsx`, `DashboardSuggestWidget.tsx`, `DashboardContactsWidget.tsx`, and `DashboardSeatsWidget.tsx`.
...
Progress update (Mar 4 2026, BUG-018 tranche 1): `LocationMap.tsx` now uses native map-click handling for **marker placement** (no DrawingManager marker mode) and shared coordinate/reverse-geocode synchronization helper. DrawingManager is still active for line/arrow/polygon/rectangle/circle and the `drawing` library is still loaded, so BUG-018 remains open until those modes are migrated and the library is removed.
