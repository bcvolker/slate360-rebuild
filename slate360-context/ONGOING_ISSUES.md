...
**Last Updated:** 2026-04-18 (R2 browser upload CORS blocker identified after app-level smoke test)  
...
| BUG-032 | SlateDrop / storage | **Cloudflare R2 browser uploads still fail after the shared client migration because the bucket itself has no browser CORS rule.** App-level smoke testing now passes bucket access, presign generation, and server-side write/delete probes, and the app CSP has been updated to allow `*.r2.cloudflarestorage.com`. The remaining failure is the browser preflight on presigned PUT URLs: `No 'Access-Control-Allow-Origin' header is present on the requested resource.` Until the bucket CORS policy is added in Cloudflare, direct browser uploads in SlateDrop/upload-portal style flows will fail before `/api/slatedrop/complete`. | High | 🔴 Open — external bucket config required |
