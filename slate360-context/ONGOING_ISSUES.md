...
| BUG-021 | Location / Multiple Views | **Location widgets are inconsistent across the platform.** `WizardLocationPicker` (project creation step 3), `LocationMap` (dashboard widget), project address display in card/list views, and Project Hub Tier 2 project home render location data with mixed contracts/styles. Progress: shared `LocationDisplay` component created and wired into dashboard/project-hub card views; remaining map + wizard alignment pending. | Medium | 🟡 In progress |
